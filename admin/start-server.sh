...
(cd frontend/ && npm run build >> "$FRONTEND_LOG" 2>&1 && npm run preview -- --port 5173 >> "$FRONTEND_LOG" 2>&1 & echo $! > frontend_pid.txt)
echo "[LOG] PID (frontend): $(cat frontend_pid.txt)"

echo "[LOG] Start backend server, log at api/$BACKEND_LOG"
(cd api/ && source .venv/bin/activate && fastapi run app.py --host 127.0.0.1 --port 8000 >> "$BACKEND_LOG" 2>&1 & echo $! > backend_pid.txt)
